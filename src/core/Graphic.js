...
const docElWidth = document.getElementsByTagName("html")[0].offsetWidth;
class BaseGraphic {
  constructor(viewer) {
...
class CesiumBillboard extends BaseGraphic {
  ...
  updateText(text, description,position) {
    if (this.graphic) {
      this.graphic.label.text = text;
      this.name = text;
      this.description = description;
      this.mname = text
      //this.graphic.position = Cesium.Cartesian3.fromDegrees(position[0], +position[1], +position[2] ? +position[2] : 0.8);
    }
  }
  updateImage(img) {
    if (this.graphic) {
  ...
  static defaultStyle = {
    ...
    image: '/static/images/jingyuan.png',
    //verticalOrigin: Cesium.VerticalOrigin.BASELINE,
    scale: docElWidth > 2400 ? 1:0.6, // 设置图标的大小
    verticalOrigin: Cesium.VerticalOrigin.BOTTOM, // 设置图标的垂直位置
      horizontalOrigin: Cesium.HorizontalOrigin.CENTER, // 设置图标的水平位置
      scaleByDistance: new Cesium.NearFarScalar(1.5e2, 2, 1.5e3, 0.5), //设置近大远小
      distanceDisplayCondition: new Cesium.DistanceDisplayCondition(10.0, 5000.0),// 设置相机距离模型多远的距离显示
  }
...
class CesiumPolygon extends BaseGraphic {
   ...
   * new Cesium.CallbackProperty(callbackFn,isConstant)
   */
  startEdit() {
   ...
   */
  static defaultStyle = {
    material: new Cesium.Color.fromCssColorString('rgba(247,224,32,0.1)'),
    outline: true,
...
class CesiumLabel extends BaseGraphic {
  ...
  create() {
    console.log('文字++++++options',this.options)
    this.graphic = this.viewer.entities.add(this.options);
    console.log('文字++++++',this.graphic)
  }
  startEdit() {
    if (this.graphic) {
  ...
  static defaultStyle = {
      text: 'test', // 设置文字内容
      font: docElWidth > 2800 ? '32px Helvetica':'20px Helvetica', // 设置文字字体
      fillColor: Cesium.Color.AQUAMARINE, // 设置文字颜色
      outlineColor: Cesium.Color.AQUAMARINE, // 设置文字轮廓颜色
      outlineWidth: 0.2, // 设置文字轮廓宽度
      style: Cesium.LabelStyle.FILL_AND_OUTLINE, // 设置文字样式
      verticalOrigin: Cesium.VerticalOrigin.BOTTOM, // 设置文字垂直位置
      horizontalOrigin: Cesium.HorizontalOrigin.CENTER, // 设置文字水平位置
      scaleByDistance: new Cesium.NearFarScalar(1.5e2, 4, 1.5e3, 0.7),
      distanceDisplayCondition: new Cesium.DistanceDisplayCondition(10.0, 5000.0),// 设置相机距离模型多远的距离显示
  }
  destroy() {
    this.remove();
