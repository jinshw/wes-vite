    ...
    <div>
        <div>
            <el-form :inline="true">
                <el-row>
                    <el-col :span="6">
                        <el-form-item label="设备杆名称">
                            <el-input v-model.trim="searchObj.name" type="text" placeholder="请输入设备杆名称" />
                        </el-form-item>
                    </el-col>
                    <el-col :span="6">
        ...
        <div class="table-box table-wrap">
            <el-table :data="tableData" height="auto">
                ...
                <el-table-column prop="deviceType" label="设备杆类型" width="120" :formatter='formatterDeviceType'/>
                <el-table-column prop="name" label="设备杆名称" width="200" />
                <el-table-column prop="poleCode" label="设备杆编码" width="200"/>
        ...
        <el-dialog :title="dialogTitle" v-model="addDialogVisible" :fullscreen="true" @open="mapOpenCallback"
            ...
            <el-form ref="addFormRef" :inline="true" label-width="110px" size="small" :rules="rules" :model="dataRow"
                ...
                <el-form-item label="设备杆名称" prop="name">
                    <el-input v-model="dataRow.name" auto-complete="off" placeholder="请输入设备杆名称" />
                </el-form-item>
                <el-form-item label="设备杆编码" prop="poleCode">
                    <el-input v-model="dataRow.poleCode" auto-complete="off" placeholder="请输入设备杆编码"/>
                </el-form-item>
                <el-form-item label="设备杆类型" prop="deviceType">
...
import {
    trafficPoleListByPage,
    addTrafficPole,
    updateTrafficPole,
    deleteTrafficPole,
    verifyTrafficPoleCommit,
...
let dataRow = reactive({ poleId:"", name: "", poleCode: "", desc:"", polygon: "", polygonCenter:"", crossId:"",deviceId:"",deviceType:""});
let rules = reactive({
    name: [
    ...
    poleCode: [{ required: true, message: "请输设备杆编码", trigger: "blur" }],
    crossId :  [{ required: true, message: "请选择所属路口", trigger: "blur" }],
    deviceType: [{ required: true, message: "请输设备杆类型", trigger: "blur" }],
...
let verifyRow = reactive({ poleId:"", collStatus:"", desc: ""});
...
const getList = (pageNum, pageSize) => {
    ...
    trafficPoleListByPage(searchObj).then((response) => {
...
const handleDelete = (index, row) => {
    ElMessageBox.confirm("此操作将永久删除该记录, 是否继续?", "提示", {
        ...
        .then(() => {
            deleteTrafficPole({ poleId: row.poleId }).then((response) => {
...
const commitEventForm = () => {
    ...
    addFormRef.value.validate((isValid, invalidFields) => {
        if (isValid) {
            if (saveType.value == 1) {
                // 新增
                addTrafficPole(dataRow).then((response) => {
                    if (response.status == 200) {
                        ElMessage({
                            message: "新增保存成功!",
                            type: "success"
                ...
                // 2:编辑
                updateTrafficPole(dataRow).then((response) => {
                    if (response.status == 200) {
                        ElMessage({
                            message: "修改保存成功!",
                            type: "success"
...
const resetForm = () => {
    dataRow = reactive({ poleId:"", name: "", poleCode: "", desc:"", polygon: "", polygonCenter:"", crossId:"",deviceId:"",deviceType:""});
    verifyRow = reactive({ poleId:"", collStatus:"", desc: ""});
    resetClearValidate();
};
...
const commitEventFactorVerify= function(){
    verifyFormRef.value.validate((isValid, invalidFields) => {
        if (isValid) {
            verifyTrafficPoleCommit(verifyRow).then(res => {
                if (res.status == 200) {
                    ElMessage({
                        type: 'success',
                        message: '操作成功!'
