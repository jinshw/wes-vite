    ...
    <div>
        <div>
            <el-form :inline="true">
                <el-row>
                    <el-col :span="6">
                        <el-form-item label="可变标志名称">
                            <el-input v-model.trim="searchObj.name" type="text" placeholder="请输入可变标志名称" />
                        </el-form-item>
                    </el-col>
                    <el-col :span="6">
                    ...
                    <el-col :span="10" class="btn-textright">
        ...
        <div class="table-box table-wrap">
            <el-table :data="tableData" height="auto">
                ...
                <el-table-column prop="deviceType" label="标志模型类型" width="120" :formatter='formatterDeviceType'/>
                <el-table-column prop="type" label="可变标志类型" width="200" :formatter='formatterChangeInfoType'/>
                <el-table-column prop="name" label="可变标志名称" width="200" />
                <el-table-column prop="code" label="可变标志编码" width="200"/>
        ...
        <el-dialog :title="dialogTitle" v-model="addDialogVisible" :fullscreen="true" @open="mapOpenCallback"
            ...
            <el-form ref="addFormRef" :inline="true" label-width="110px" size="small" :rules="rules" :model="dataRow"
                ...
                <el-form-item label="可变标志名称" prop="name">
                    <el-input v-model="dataRow.name" auto-complete="off" placeholder="请输入可变标志名称" />
                </el-form-item>
                <el-form-item label="可变标志编码" prop="code">
                    <el-input v-model="dataRow.code" auto-complete="off" placeholder="请输入可变标志编码"/>
                </el-form-item>
                <el-form-item label="可变标志类型" prop="type">
                    <el-select class="inputW" v-model="dataRow.type" filterable allow-create
                        default-first-option placeholder="请选择">
                        <el-option v-for="item in changeInfoTypeList" :key="item.code" :label="item.name"
                            :value="item.code" />
                    </el-select>
                </el-form-item>
                <el-form-item label="标志模型类型" prop="deviceType">
                    <el-select class="inputW" v-model="dataRow.deviceType" filterable allow-create
                        default-first-option placeholder="请选择" @change="deviceTypeChange">
                        <el-option v-for="item in mapDeviceTypes" :key="item.dtId" :label="item.typeName"
                            :value="item.dtId"></el-option>
                    </el-select>
                </el-form-item>
                ...
                <el-form-item>
                    ...
                    <el-button type="primary" size="small" @click="lightSimulate" v-show="dataRow.deviceType == 16">信号灯模拟</el-button>
                </el-form-item>
            </el-form>
            <div>
...
import {
    changeInfoMarkListByPage,
    addChangeInfoMark,
    updateChangeInfoMark,
    deleteChangeInfoMark,
    verifyChangeInfoMarkCommit,
...
let dataRow = reactive({ id:"", name: "", code: "", desc:"", polygon: "", polygonCenter:"", crossId:"",deviceId:"",deviceType:"",type:"",location:"",manufacturer:""});
let rules = reactive({
    name: [
    ...
    code: [{ required: true, message: "请输可变标志编码", trigger: "blur" }],
    crossId :  [{ required: true, message: "请选择所属路口", trigger: "blur" }],
    deviceType: [{ required: true, message: "请输可变标志类型", trigger: "blur" }],
...
const trafficPoleList = ref([]);
const mapDeviceList = ref([]);
const mapDeviceTypes = ref([]);
...
let verifyRow = reactive({ id:"", collStatus:"", desc: ""});
const changeInfoTypeList = ref([{"code":"10","name":"可变情报板"},{"code":"11","name":"LED可变信息标志"},{"code":"12","name":"可变交通标志"},{"code":"13","name":"信号机"}]);

onMounted(() => {
    getAssetCrossingDataList();
    getMapDeviceTypeList();
    search();
});
...
const getList = (pageNum, pageSize) => {
    ...
    changeInfoMarkListByPage(searchObj).then((response) => {
        tableData.value = response.data.list;
        console.log("tableData.value = response.data.list;",tableData.value)
...
const getMapDeviceTypeList= () => {
    mapDeviceTypeList({}).then((response) => {
        mapDeviceTypes.value = response.data;
        console.log("mapDeviceTypes.value==",mapDeviceTypes.value);
    });
};


const deviceTypeChange= (val) => {
    mapDeviceListByDeviceType({"deviceType":val}).then((response) => {
        mapDeviceList.value = response.data;
        console.log("mapDeviceList.value==",mapDeviceList.value);   
    });
};

const getMapDeviceListByDeviceType= () => {
    mapDeviceListByDeviceType({"deviceTypeCode":dataRow.deviceType}).then((response) => {
        mapDeviceList.value = response.data;
    });
};
...
const handleEdit = (index, row) => {
    ...
    deviceTypeChange(dataRow.deviceType);
    if(dataRow.crossId){
...
const initRowToObj = (row) => {
    ...
    dataRow.deviceType = row.deviceType;
...
const handleDelete = (index, row) => {
    ElMessageBox.confirm("此操作将永久删除该记录, 是否继续?", "提示", {
        ...
        .then(() => {
            deleteChangeInfoMark({ id: row.id }).then((response) => {
...
const commitEventForm = () => {
    ...
    addFormRef.value.validate((isValid, invalidFields) => {
        if (isValid) {
            if (saveType.value == 1) {
                // 新增
                addChangeInfoMark(dataRow).then((response) => {
                    if (response.status == 200) {
                        ElMessage({
                            message: "新增保存成功!",
                            type: "success"
                ...
                // 2:编辑
                updateChangeInfoMark(dataRow).then((response) => {
                    if (response.status == 200) {
                        ElMessage({
                            message: "修改保存成功!",
                            type: "success"
...
const resetForm = () => {
    dataRow = reactive({ id:"", name: "", code: "", desc:"", polygon: "", polygonCenter:"", crossId:"",deviceId:"",deviceType:"",type:"",location:"",manufacturer:""});
    verifyRow = reactive({ id:"", collStatus:"", desc: ""});
    resetClearValidate();
};
...
const formatterDeviceType = (row, column, cellValue, index)=> {
    const deviceType = row[column.property];
    let name = "";
    if (deviceType) {
        let rData = mapDeviceTypes.value
        for (let i = 0; i < rData.length; i++) {
            if (rData[i].dtId == deviceType) {
                name = rData[i].typeName;
            }
        }
    }
    return name;
...
const formatterChangeInfoType = (row, column, cellValue, index)=> {
    const type = row[column.property];
    let name = "";
    if (type) {
        let rData = changeInfoTypeList.value
        for (let i = 0; i < rData.length; i++) {
...
const commitEventFactorVerify= function(){
    verifyFormRef.value.validate((isValid, invalidFields) => {
        if (isValid) {
            verifyChangeInfoMarkCommit(verifyRow).then(res => {
                if (res.status == 200) {
                    ElMessage({
                        type: 'success',
                        message: '操作成功!'
