    ...
    <div>
                ...
                <el-row>
                    <el-col :span="18">
                        <el-form-item label="路段名称">
                            <el-input v-model.trim="searchObj.name" type="text" placeholder="请输入路段名称" />
                        </el-form-item>
                    </el-col>
        ...
        <div class="table-box table-wrap">
            <el-table :data="tableData" height="auto">
                ...
                <el-table-column prop="roadId" label="所属路线" width="150" :formatter='formatterRoadId'/>
                <el-table-column prop="name" label="路段名称" width="200" />
                <el-table-column prop="sectionCode" label="路段编码" width="200"/>
                <el-table-column prop="locationStart" label="路段起点" width="200"/>
                <el-table-column prop="locationEnd" label="路段终点" width="200"/>
                <el-table-column prop="sectionLevel" label="道路等级" width="200"/>
                <el-table-column prop="optTime" label="更新时间" width="160" :formatter="formatDate"  />
        ...
        <el-dialog :title="dialogTitle" v-model="addDialogVisible" :fullscreen="true" @open="mapOpenCallback"
            ...
            <el-form ref="addFormRef" :inline="true" label-width="110px" size="small" :rules="rules" :model="dataRow"
                class="dialogElForm">
                <el-form-item label="所属路线" prop="roadId">
                    <el-select class="inputW" v-model="dataRow.roadId" filterable allow-create
                        default-first-option placeholder="请选择">
                        <el-option v-for="item in roadDataList" :key="item.roadId" :label="item.name"
                            :value="item.roadId"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="路段名称" prop="name">
                    <el-input v-model="dataRow.name" auto-complete="off" placeholder="请输入路段名称" />
                </el-form-item>
                <el-form-item label="路段编码" prop="sectionCode">
                    <el-input v-model="dataRow.sectionCode" auto-complete="off" placeholder="请输入路段编码"/>
                </el-form-item>
                <el-form-item label="区域范围" prop="polygon">
                    <el-input v-model="dataRow.polygon" auto-complete="off" placeholder="请输入区域范围" disabled />
                </el-form-item>
                <el-form-item label="上行区域" prop="directionLeft">
                    <el-input v-model="dataRow.directionLeft" auto-complete="off" placeholder="请输入区域范围" disabled />
                </el-form-item>
                <el-form-item label="下行区域" prop="directionRight">
                    <el-input v-model="dataRow.directionRight" auto-complete="off" placeholder="请输入区域范围" disabled />
                </el-form-item>
                <el-form-item label="路段起点" prop="locationStart">
                    <el-input v-model="dataRow.locationStart" auto-complete="off" placeholder="请输入路段起点"/>
                </el-form-item>
                <el-form-item label="路段终点" prop="locationEnd">
                    <el-input v-model="dataRow.locationEnd" auto-complete="off" placeholder="请输入路段终点"/>
                </el-form-item>
                <el-form-item label="道路等级" prop="sectionLevel">
                    <el-input v-model="dataRow.sectionLevel" auto-complete="off" placeholder="请输入道路等级"/>
                </el-form-item>
...
import { MapLocation, Location ,Crop} from '@element-plus/icons-vue'
...
let dataRow = reactive({ sectionId:"", name: "", sectionCode: "", polygon: "", lineColor:"", lineStyle:"", lineWidth:"", outlineColor:"", outlineStyle:"", outlineWidth:"", 
                polygonCenter:"", desc:"", locationStart:"", locationEnd:"", sectionLevel:"", roadId:"",id:"",directions:[],directionLeft:"",directionRight:""});
let rules = reactive({
    name: [
    ...
    sectionCode: [{ required: true, message: "请输路段编码", trigger: "blur" }],
    polygon: [{ required: true, message: "请选择区域范围", trigger: "blur" }],
    roadId: [{ required: true, message: "请选择所属路线", trigger: "blur" }],
    directionLeft: [{ required: true, message: "请选择区域范围", trigger: "blur" }],
    directionRight: [{ required: true, message: "请选择区域范围", trigger: "blur" }]
});
...
const getList = (pageNum, pageSize) => {
    ...
    assetSectionListByPage(searchObj).then((response) => {
        tableData.value = response.data.list;
        totalCount = response.number;
    });
};

const getAssetAssetRoadList= () => {
    assetAssetRoadList({}).then((response) => {
        roadDataList.value = response.data;
    });
};
...
const initRowToObj = (row) => {
    dataRow.sectionId = row.sectionId;
    dataRow.name = row.name;
    dataRow.sectionCode = row.sectionCode;
    ...
    dataRow.sectionLevel = row.sectionLevel;
    dataRow.roadId = row.roadId;
    dataRow.id = row.id;
    dataRow.directions = row.directions;
    dataRow.directionLeft =row.directionLeft;
    dataRow.directionRight =row.directionRight;
    if(dataRow.directions.length>0){
        dataRow.directions.forEach(item => {
            if(item.directionType=="2"){
                dataRow.directionLeft= item.polygon;
            }else if(item.directionType=="3"){
                dataRow.directionRight= item.polygon;
            }
        })
    }
};
...
const handleDelete = (index, row) => {
    ElMessageBox.confirm("此操作将永久删除该记录, 是否继续?", "提示", {
        ...
        .then(() => {
            deleteAssetSection({ sectionId: row.sectionId }).then((response) => {
...
const commitEventForm = () => {
    ...
    addFormRef.value.validate((isValid, invalidFields) => {
        if (isValid) {
            if (saveType.value == 1) {
                // 新增
                addAssetSection(dataRow).then((response) => {
                    if (response.status == 200) {
                        ElMessage({
                            message: "新增保存成功!",
                            type: "success"
                ...
                // 2:编辑
                updateAssetSection(dataRow).then((response) => {
                    if (response.status == 200) {
                        ElMessage({
                            message: "修改保存成功!",
                            type: "success"
...
const resetForm = () => {
    dataRow = reactive({sectionId:"", name: "", sectionCode: "", polygon: "", lineColor:"", lineStyle:"", lineWidth:"", outlineColor:"", outlineStyle:"", outlineWidth:"", 
        polygonCenter:"", desc:"", locationStart:"", locationEnd:"", sectionLevel:"",roadId:"",id:"",directions:[],directionLeft:"",directionRight:""});
    resetClearValidate();
};
...
const formatterRoadId = (row, column, cellValue, index)=> {
    const roadId = row[column.property];
    let roadName = "";
    if (roadId) {
        let rData = roadDataList.value
        for (let i = 0; i < rData.length; i++) {
            if (rData[i].roadId == roadId) {
                roadName = rData[i].name;
            }
        }
    }
    return roadName;
...
const loadCurrDataRow = () => {
    ...
    if(dataRow.directions.length>0){
        let directions = dataRow.directions;
        for(let i=0;i<directions.length;i++){
            let dirObj = {};
            dirObj.id = directions[i].id;
            dirObj.name = directions[i].name;
            dirObj.directionCode = directions[i].directionCode;
            dirObj.directionType = directions[i].directionType;
            dirObj.polygon = directions[i].polygon;
            dirObj.polygonCenter = directions[i].polygonCenter;
            dirObj.lineColor = directions[i].lineColor;
            dirObj.lineStyle = directions[i].lineStyle;
            dirObj.lineWidth = directions[i].lineWidth;
            dirObj.outlineColor = directions[i].outlineColor;
            dirObj.outlineStyle = directions[i].outlineStyle;
            dirObj.outlineWidth = directions[i].outlineWidth;
            currDataList.push(dirObj);
        }
    }
    if (viewer) {
...
const handleChildEmit = (data) =>{
    if(data.points.length>0){
        ...
        }else if((['2', '3']).includes(data.areaPolygonType)){
            let areaTypeArr = [];
            dataRow.directions.forEach(item => {
                areaTypeArr.push(item.directionType);
            })
            if(areaTypeArr.includes(data.areaPolygonType)){
                dataRow.directions.forEach(item => {
                    if(item.directionType==data.areaPolygonType){
                        item.polygon = JSON.stringify(data.points)
                        item.lineColor = data.lineColor
                        item.outlineColor = data.outlineColor
                        item.outlineWidth = data.outlineWidth
                    }
                })
            }else{
                let dirObj = {};
                dirObj.directionId = data.id;
                dirObj.id = data.id;
                dirObj.name = data.name;
                dirObj.directionCode = data.areaPolygonType;
                dirObj.directionType = data.areaPolygonType;
                dirObj.polygon = JSON.stringify(data.points);
                dirObj.lineColor = data.lineColor
                dirObj.outlineColor = data.outlineColor
                dirObj.outlineWidth = data.outlineWidth
                dataRow.directions.push(dirObj);
            }
            if(data.areaPolygonType=="2"){
                dataRow.directionLeft= JSON.stringify(data.points);
            }
            if(data.areaPolygonType=="3"){
                dataRow.directionRight= JSON.stringify(data.points);
            }
        }
        loadCurrDataRow();
    }else{
