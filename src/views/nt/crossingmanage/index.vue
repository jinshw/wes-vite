    ...
    <div>
                ...
                <el-row>
                    <el-col :span="18">
                        <el-form-item label="路口名称">
                            <el-input v-model.trim="searchObj.name" type="text" placeholder="请输入路口名称" />
                        </el-form-item>
                    </el-col>
        ...
        <div class="table-box table-wrap">
            <el-table :data="tableData" height="auto">
                ...
                <el-table-column prop="name" label="路口名称" width="200" />
                <el-table-column prop="crossCode" label="路口编码" width="200"/>
        ...
        <el-dialog :title="dialogTitle" v-model="addDialogVisible" :fullscreen="true" @open="mapOpenCallback"
            ...
            <el-form ref="addFormRef" :inline="true" label-width="190px" size="small" :rules="rules" :model="dataRow"
                ...
                <el-form-item label="路口名称" prop="name">
                    <el-input v-model="dataRow.name" auto-complete="off" placeholder="请输入路口名称" />
                </el-form-item>
                <el-form-item label="路口编码" prop="crossCode">
                    <el-input v-model="dataRow.crossCode" auto-complete="off" placeholder="请输入路口编码"/>
                </el-form-item>
...
import {
    assetCrossingDataListByPage,
    addAssetCrossingData,
    updateAssetCrossingData,
    deleteAssetCrossingData,
    assetAssetRoadList
...
let dataRow = reactive({ crossId:"", name: "", crossCode: "", polygon: "", lineColor:"", lineStyle:"", lineWidth:"", 
                        outlineColor:"", outlineStyle:"", outlineWidth:"", polygonCenter:"", desc:"", locationStart:"", locationEnd:"", roadLevel:"", roadId:"",id:""});
let rules = reactive({
    name: [
    ...
    crossCode: [{ required: true, message: "请输路口编码", trigger: "blur" }],
...
const getList = (pageNum, pageSize) => {
    ...
    assetCrossingDataListByPage(searchObj).then((response) => {
...
const initRowToObj = (row) => {
    ...
    dataRow.crossCode = row.crossCode;
...
const handleDelete = (index, row) => {
    ElMessageBox.confirm("此操作将永久删除该记录, 是否继续?", "提示", {
        ...
        .then(() => {
            deleteAssetCrossingData({ crossId: row.crossId }).then((response) => {
...
const commitEventForm = () => {
    ...
    addFormRef.value.validate((isValid, invalidFields) => {
        if (isValid) {
            if (saveType.value == 1) {
                // 新增
                addAssetCrossingData(dataRow).then((response) => {
                    if (response.status == 200) {
                        ElMessage({
                            message: "新增保存成功!",
                            type: "success"
                ...
                // 2:编辑
                updateAssetCrossingData(dataRow).then((response) => {
                    if (response.status == 200) {
                        ElMessage({
                            message: "修改保存成功!",
                            type: "success"
...
const resetForm = () => {
    dataRow = reactive({ crossId:"", name: "", crossCode: "", polygon: "", lineColor:"", lineStyle:"", lineWidth:"", 
                         outlineColor:"", outlineStyle:"", outlineWidth:"", polygonCenter:"", desc:"", locationStart:"", locationEnd:"", roadLevel:"", roadId:"",id:""});
    resetClearValidate();
};
