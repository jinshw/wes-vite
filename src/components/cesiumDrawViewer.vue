<template>
  <div >
    <el-container id="drawtoolPanel">
      <el-header id="drawtoolHead" style="display:none;">
        <span>基础标绘</span>
      </el-header>
        ...
        <ul>
          <!-- <li>
            <i class="cesiumDrawFont iconmarker icon-class" title="添加标记"
              :class="{ 'selected-graphic': menuSelected['MARKER'] }" @click.stop="menuAction('MARKER')"></i>
            <span :class="{ 'selected-graphic': menuSelected['MARKER'] }">标记</span>
          </li> 
          <li>
            <i class="cesiumDrawFont iconpolyline icon-class" title="添加线段"
              :class="{ 'selected-graphic': menuSelected['POLYLINE'] }" @click.stop="menuAction('POLYLINE')"></i>
          ...
          <li>
            <i class="cesiumDrawFont iconpolygon icon-class" title="添加多边形"
              :class="{ 'selected-graphic': menuSelected['POLYGON'] }" @click.stop="menuAction('POLYGON')"></i>
          ...
          <!-- <li>
            <i class="cesiumDrawFont iconlabel icon-class" title="添加文字"
              :class="{ 'selected-graphic': menuSelected['LABEL'] }" @click.stop="menuAction('LABEL')"></i>
            <span @click="menuAction('LABEL')" :class="{ 'selected-graphic': menuSelected['LABEL'] }">文字</span>
          </li>  -->
          <!-- <li v-show="extendMarkerModel.length">
            <i class="cesiumDrawFont iconmodel icon-class" title="添加模型"
              :class="{ 'selected-graphic': menuSelected['MODEL'] }" @click.stop="menuAction('MODEL')"></i>
            <span  :class="{ 'selected-graphic': menuSelected['MODEL'] }">模型</span>
          </li> -->
          <!-- <li>
            <i
              class="font-btn"
              ...
              @click.stop="menuAction('LABEL')"
            ></i>
            <span @click="menuAction('LABEL')">文字</span>
          </li>-->
          <li style="display:none;">
            <i class="cesiumDrawFont iconlayer icon-class" :class="{ 'selected-graphic': layerManagerVisible }"
              title="图层管理" @click.stop="layerManagerVisible = !layerManagerVisible"></i>
            <span :class="{ 'selected-graphic': layerManagerVisible }">清单</span>
          </li>
        </ul>
      ...
      <!-- <div class="marker-edit-class edit-class" v-show="menuSelected['MARKER']">
        ...
        <el-select size="small" v-model="fontSize" allow-create filterable title="字号" default-first-option
          placeholder="请选择">
      ...
      <div class="polyline-edit-class edit-class" v-show="menuSelected['POLYLINE']">
        ...
        <el-select size="small" title="线宽" class="width-selector" v-model="lineWidth" allow-create filterable
          default-first-option placeholder="请选择">
          <el-option v-for="item in lineWidthList" :key="item" :label="item" :value="item"></el-option>
        </el-select>
        <el-select size="small" class="style-selector" title="直线样式" v-model="lineStyle" default-first-option
          placeholder="请选择">
          <el-option v-for="item in lineStyleList" :key="item.value" :label="item.name" :value="item.value"></el-option>
        </el-select>
        <el-select size="small" title="直线类型" class="type-selector" v-model="graphicHeight" default-first-option
          placeholder="请选择" style="display:none;">
          <el-option v-for="item in heightList" :key="item.value" :label="item.name" :value="item.value"></el-option>
        </el-select>
        <el-select size="small" title="数据类型" class="type-selector" v-model="assetDictType" default-first-option
          placeholder="请选择" @change="assetDictTypeChange" v-show="lineStyle=='glow'">
          <el-option v-for="item in assetDictLineList" :key="item.value" :label="item.name" :value="item.value"></el-option>
        </el-select>
      </div>
      <div class="polygon-edit-class edit-class" v-show="menuSelected['POLYGON']">
        ...
        <el-select size="small" title="多边形类型" class="type-selector" v-model="graphicHeight" default-first-option
          placeholder="请选择" style="display:none;">
          <el-option v-for="item in heightList" :key="item.value" :label="item.name" :value="item.value"></el-option>
        </el-select>
        <i class="cesiumDrawFont iconoutline border-btn" :class="{ 'outline-selected': outline }" title="边框"
          @click="outline = !outline"></i>
        <div v-show="outline">
          <el-color-picker title="边框充色" id="outlineColor" v-model="outlineColor" show-alpha
            size="small"></el-color-picker>
        </div>
        <el-select size="small" title="边框宽度" v-show="outline" class="width-selector" v-model="outlineWidth" allow-create
          filterable default-first-option placeholder="请选择">
          <el-option v-for="item in lineWidthList" :key="item" :label="item" :value="item"></el-option>
        </el-select>
        <el-select size="small" title="数据类型" class="type-selector" v-model="areaPolygonType" default-first-option
          placeholder="请选择" @change="areaPolygonTypeChange">
          <el-option v-for="item in areaPolygonList" :key="item.value" :label="item.name" :value="item.value"></el-option>
        </el-select>
      </div>
      <div class="label-edit-class edit-class" v-show="menuSelected['LABEL']">
        <el-select v-model="fontFamily" class="font-selector" allow-create filterable size="small" title="字体"
          default-first-option placeholder="请选择">
          <el-option v-for="item in fontList" :key="item" :label="item" :value="item"></el-option>
        </el-select>
        <el-select size="small" v-model="fontSize" class="size-selector" allow-create filterable title="字号"
          default-first-option placeholder="请选择">
          <el-option v-for="item in fontSizeList" :key="item" :label="item" :value="item"></el-option>
        </el-select>
      ...
      <div class="model-edit-class edit-class" v-show="menuSelected['MODEL']">
        <!--<el-popover placement="bottom" id="model-select" width="160" v-model="modelSelectPanelvisible">
          <div class="model-select-panel">
            <img v-for="item in extendMarkerModel" :title="item.name" :key="item.id" @click="selectModel(item)"
              :src="item.thumb" :onerror="defaultImage" />
          </div>
          <template #reference>
        ...
        <el-slider v-show="modelMode === 'Mix'" title="混合度" v-model="modelMixed" :min="0" :max="1" :step="0.1"
    ...
    <MarkerViewer ref="markerManager" :actplanId = "yuanListSelected" :attachment="attachment" @deleteEvent="deleteMarker" @editEvent="editMarker"
      @addEvent="addMarker" @updateEvent="updateMarker" :extendImage="extendMarkerImage"></MarkerViewer>
    <layerManager ref="layerManager" @locate="locateGraphic" @edit="editGraphic" @delete="deleteGraphic"
      @rename="renameGraphic" @select="selectGraphic" @clear="clearGraphic" @close="closeLayerManager"
      @import="importGraphic" @export="exportGraphic" :tools="tools" v-show="layerManagerVisible"
...
<script>
import { selectAssetDictListByActplan, deleteAssetDataById ,addAndUpdateActplanAssetData} from "@/api/nt/algorithm-data";
import GraphicManager from "@/core/GraphicManager.js";
import MarkerViewer from "@/components/markerViewer.vue";
import { CesiumPolygon } from "@/core/Graphic.js";
import layerManager from "@/components/layerManager.vue";
import GraphicType from "@/core/GraphicType.js";
...
export default {
  ...
  data() {
    return {
      yuanListSelected: '1',
      initEntityDatas: [],
      initEntityPolylineDatas: [],
      initEntityPolygonDatas: [],
      visible: true,
      editMode: false,
      ...
      polygonColor: "rgba(247,224,32,0.1)",
      outline: true,
      imags: [
      ...
      assetDictType:'line1',//线数据类型
      assetDictLineList: [
        {"name":"道路通行线","value":"line1"},
        {"name":"停车场进口线","value":"line2"},
        {"name":"停车场出口线","value":"line3"},
      ],//线数据各类型集合
      areaPolygonType:'1',//面数据类型
      areaPolygonList: [
        {"name":"全区域","value":"1"},
        {"name":"上行","value":"2"},
        {"name":"下行","value":"3"},
  ...
  props: {
    ...
    extendMarkerModel: {
      ...
      default: function () {
        return [{
          url:  VITE_PUBLIC_PATH +'/static/gltf/shuimadun.gltf',
          name: "防撞桶",
          id: "1",
          thumb:  VITE_PUBLIC_PATH +"/static/gltf/shuimadun.png"
        }];
      }
    },
    tools: {
  ...
  methods: {
    assetDictTypeChange(val){
      graphicManager.loadTransfeParam(this.yuanListSelected,val);
    },
    areaPolygonTypeChange(val){
      graphicManager.loadTransfeParamPloy(val);
    },
    yuanListSelectedChange(val) {
      this.yuanListSelected = val;
      this.clearGraphic();
      this.$refs.layerManager.initListJson()
      this.initEntityDatas = [];
      this.initEntityPolylineDatas = [];
      this.initEntityPolygonDatas = [];
    },
    /**
     * 初始化资产Tree数据
     * @param {*} data 
     * @param {*} node 
     */
    async initTreeData(currDataList) {
      this.clearGraphic();
      this.$refs.layerManager.initListJson()
      this.initEntityPolygonDatas = [];
      const self = this;
      graphicManager.loadTransfeParamPloy(this.areaPolygonType);
      currDataList.forEach(itemObj => {
        let pointCenter = typeof(itemObj.polygonCenter)=="undefined" || itemObj.polygonCenter==""?[0,0]:JSON.parse(itemObj.polygonCenter)
        this.initEntityPolygonDatas.push({
          id: itemObj.id,
          points: JSON.parse(itemObj.polygon),
          parentEntityType: "polygon",
          text: itemObj.name,
          name: itemObj.name,
          position: {
            lon: pointCenter[0],
          ...
          lineColor: itemObj.lineColor,
          lineWidth: itemObj.lineWidth,
          lineStyle: itemObj.lineStyle,
          outlineColor: itemObj.outlineColor,
          outlineWidth: itemObj.outlineWidth,
          outlineStyle: itemObj.outlineStyle
        })
      });
      
      //加载面数据
      if(this.initEntityPolygonDatas.length>0){
        this.initPolygonDatas(this.initEntityPolygonDatas);
      }
    },
    //加载面
    initPolygonDatas(polygonList){
      const self = this;
      polygonList.forEach(item => {
        let type = 0; //0 - marker, 4 - model， 1-polyline, 2-polygon
        if (item.parentEntityType === "polygon") {
          type = 2
        }
        self.$refs.layerManager.insertLayer(type, item.id, item.name);
      });
      self.heightList[3].name = "空间面";
      graphicManager.loadPolygonLayer(polygonList);
    },
    //线
    initPolylineDatas(polyDataList){
      const self = this;
      polyDataList.forEach(item => {
        let type = 0; //0 - marker, 4 - model， 1-polyline
        if (item.parentEntityType === "polyline") {
          type = 1
        }
        self.$refs.layerManager.insertLayer(type, item.id, item.name);
      });
      self.heightList[3].name = "空间线";
      graphicManager.heightReference = self.graphicHeight;
      graphicManager.material = Cesium.Color.fromCssColorString(
        self.lineColor
      );
      graphicManager.loadPolylineLayer(polyDataList);
    },
    ...
    init(viewer) {
      ...
      document.addEventListener("addEvent", function (e) {
        if (
          ...
          self.pushLayerManaer(e.detail.mtype, e.detail.mid, e.detail.mname,e.detail.customProperty);
        }
      });
      document.addEventListener("stopEdit", function (e) {
        if(e.changeDataRow){
          self.$emit('dataSent', e.changeDataRow);
        }
        self.menuSelected = {};
        self.editMode = false;
    ...
    pushLayerManaer(type, id, name,customProperty) {
      checkComponent(this)
      this.$refs.layerManager.insertLayer(type, id, name,customProperty);
    },
    modelThumb(item) {
      if (item.thumb) {
        return item.thumb;
    ...
    //更新列表名称
    updateMarker(mid, mname,customProperty) {
      console.log(customProperty,'=====更新列表customProperty')
      checkComponent(this)
      if (mid) {
        mname = mname || "未命名";
        this.$refs.layerManager.rename(null, mid, mname);
        //更新实体位置
        //this.$refs.markerManager.updateEntityPosition(mid,customProperty.points)
      }
      this.editMode = false;
      this.menuSelected = {};
    },
    addMarker(mid, mname, mtype,customProperty) {
      console.log(customProperty,'=====新增customProperty')
      checkComponent(this)
      this.$refs.markerManager.setCustomProperty(mid,customProperty)
      this.pushLayerManaer(mtype, mid, mname,customProperty);
    ...
    editMarker(type,customProperty) {
      checkComponent(this)
      this.editMode = true;
      // this.stopOthers();
    ...
    deleteGraphic(id) {
      // deleteAssetDataById({ 'id': id }).then(response => {
...
#drawtoolPanel {
  ...
  width: 300px;
  height: 120px;
  background: rgba(38,51,67,0.5);
  box-shadow: inset 0px 0px 5px 1px rgba(147,196,226,0.3);
